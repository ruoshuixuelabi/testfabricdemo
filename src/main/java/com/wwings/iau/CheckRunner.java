...
//	String CHAIN_CODE_LANG = TransactionRequest.Type.JAVA; //language is Java.
	...
	public void checkConfig() throws Exception{
		String ss=FileUtils.readFileToString(new File("e://HFCSampletest1.properties"),"UTF-8");
		testSampleOrgs=gson.fromJson(ss,new TypeToken<Collection<SampleOrg>>() {}.getType());
		if(testSampleOrgs!=null&&testSampleOrgs.size()>0){
			System.out.println("检查配置文件的时候不是空的");
		}else{
			//获得整合的测试的简单组织Orgs
...
//		String ss=FileUtils.readFileToString(new File("e://HFCSampletest1.properties"),"UTF-8");
//		testSampleOrgs=gson.fromJson(ss,new TypeToken<Collection<SampleOrg>>() {}.getType());
		if(testSampleOrgs!=null&&testSampleOrgs.size()>0&&testSampleOrgs.iterator().next().getAdmin()!=null){
		...
		FileUtils.writeByteArrayToFile(new File("e://HFCSampletest1.properties"),gson.toJson(testSampleOrgs).getBytes());
	}
	/**
	 * 真正运行的方法
	 */
	public void runFabricTest(final SampleStore sampleStore) throws Exception {
		...
		if(sampleStore.getChannel(client, FOO_CHANNEL_NAME)!=null){
